...
export const Button = ({label,onPress, linkTo,icon, type="button",disabled=false}) => {
    //const navigate = useNavigate();
    return(
        <Fragment>
            {
             onPress ? (<ButtonStyled disabled={disabled} type={type} onClick = {onPress}>{label}  </ButtonStyled>   )
